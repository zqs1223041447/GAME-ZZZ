...
| S4R | Direction Readiness & Baseline Lock（**inter-cycle 治理**：S4 基线冻结 + Ledger/Matrix 对账 + 下一方向决策包；**不授权任何新 gameplay capability，Runtime feature surface 冻结**） | **IN PROGRESS（2026-09-09 规划 AI 下发）**：计划=`docs/reviews/S4R/S4R_PLAN.md`（Phase 0 对账→Phase 1 Backlog 分类→Phase 2 决策包→Phase 3 **Director Direction Gate**→Phase 4 下一周期 seed）；WO-01=Frozen Baseline & Governance Reconciliation（**Gate Review=ACCEPT**，冻结基线/首次正式建立 Capability Ledger 与 Mechanic Matrix/SoT Index）；WO-02=Backlog & Dependency Normalization（候选池 32 项去重分类+依赖图+Reference Build 候选 3 例=CANDIDATE/NOT LOCKED，`S4R_BACKLOG_NORMALIZATION.md`/`S4R_REFERENCE_BUILD_CANDIDATES.md`——**候选项≠承诺实施**）；**下一实施周期=Director-Gated**——downstream 各 forbidden 项仍未授权，候选项的「存在」不构成「承诺实施」 |
