...
| 代表怪 | **Lion Head Monster / 巨魔×2 / 狼 / 蝙蝠×2** | **Dexsoft 巨魔战士（Troll_2）= PREFERRED STYLE ANCHOR — RUNTIME INTEGRATED（Art Trial R2 试装 2026-09-08 + R3 正式 Runtime 集成 2026-09-08）**：R2 九轴=8 PASS+Style Fit CONDITIONAL（现由规划 AI 裁定落地=普通/精英敌人默认视觉锚点，Bruce 转 Role-Specific Boss/高价值候选=DECISIONS 规则⑫）；R3 经通用表现层 `EnemyVisualPresenter` 正式接入默认 Runtime Brute 视觉位（动画五类由真实 gameplay 信号驱动/gameplay·Content·UI·Audio delta=0/EditMode 169/169+PlayMode 5/5/PlayerRuntime Gate PASS/Performance Gate PASS 9/9 worst p99=2.615ms=31.4% 且如实记录 Harness 代表性=NO/构建含 Troll 资产/resources.assets 检索实证）；来源=巨魔战士.rar→Dexsoft Troll Warrior/troll2.unitypackage（许可不再 Gate=规则⑪）；剩余候选=Lion Head / TheTroll（另一巨魔包）/ 狼 / 蝙蝠×2 保持 SCREENED/NOT STARTED | Troll_2 覆盖「近战重甲」代表怪位并已为默认敌人视觉；其余候选待规划 AI 决定同风格批量筛选或补角色位 |
