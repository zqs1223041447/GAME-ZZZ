...
本轮 commit（Phase 5 Art Trial R8）：A=docs(art) Ashling 定向选模（ASHLING_MODEL_SCREENING_R8.md：全池盘点+筛选轴+SELECTED=SFB Gargoyle）+ art(s3) 导入候选资产（Assets/Art/Enemies/Gargoyle/：FBX+三贴图+材质+控制器+GargoyleVisual.prefab 保留于 Prefabs/ 非 Resources 入口）+ feat(art) 集成尝试后按工作令回退（四视觉 Art Gate 实测 FAIL→**Ashling 恢复 placeholder**）+ test(art) 保留资产健康测试 7 项（AshlingVisualContractTests）+ docs(art) R8 复核报告+STATUS 回填；**最终 Verdict=ASSET ACCEPTED — ASHLING RUNTIME INTEGRATION BLOCKED BY ART PERFORMANCE**（200 档 p99≈10.8、300 档 avg≈9.1/p99≈14.9 超预算，3 runs 全 FAIL；不降质修绿，恢复后三视觉基线 -IncludeArtPerformance 全 PASS 无回归）；上轮 R7 五笔（77360f2+321d9b4+4ada5bf+f2f6d16 等）已回填于下方绿灯表 / B=回填本行
