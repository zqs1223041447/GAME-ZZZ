...
本轮 commit（S5-WO-01-APPROVED-SCOPE-AND-CONTRACT-LOCK）：**S5 正式启动（AUTHORIZED FOR PHASE 0）——Phase 0 合同锁定完成（纯文档轮，Runtime delta=NONE）**。链路：导演原子批准（BL-002.A1 新增现有类型词缀=APPROVE；BL-021.A2 一件装备多组连接=APPROVE；BL-021.A1 孔颜色/BL-012.A1 宝石等级品质=明确不做）→ 规划 AI 下发 S5 计划+WO-01 → 工作 AI 执行：**S5_PLAN.md 落库**（周期定义/硬边界/Phase 0-5/确定性规则：S5 Final 须建立新 sim hash 并 3 次精确重复）+ **`S5_LINK_CONTRACT.md`**（MaxLinkGroupsPerEligibleItem=2 唯一权威合同：group 1=末尾 2 孔「技能改挂」语义——被改挂技能恰一个连接源、无跨组泄漏；数据模型=ItemInstance 单字段 LinkSkill1（SkillId.None=0=legacy 等价）；2 组资格由 SocketCount≥3 推导（当前=Weapon/Body）；12 场景测试矩阵+golden oracle 双组位复用策略+禁区护栏）+ **`S5_AFFIX_ADMISSION.md`**（**N=4 锁定**：迅疾=AttackSpeed Inc 通用 / 铁骨=Armour Inc 非 Belt（保留壁垒生态位）/ 睿智=Intelligence Flat / 开阔=AreaRadiusMore More；17→21；全部既有 StatId/ModOp、单一 applicability、随机池+定向制作自动可达、PoEDB 人工溯源无 BL-024；Phase 3 须同步更新 S4-P3 计数护栏 17→21）+ **`S5_SCOPE_LEDGER.md`**（APPROVED/REJECTED/NOT AUTHORIZED 三区台账）+ DECISIONS 登记。**同轮自纠 1 处自引漂移**：S4R 轮 RUNTIME 误写「Gloves/Belt 无 Socket」→ 修正为「各 1 孔、仅展示计数、不映射技能孔位」（SliceRules GlovesSockets/BeltSockets=1 实证；Ledger 同步）。Ledger/Matrix 仅授权跟踪不晋升（Multiple Link Groups=AUTHORIZED NOT IMPLEMENTED）；孔色/宝石等级品质=显式拒绝负向条件。**Runtime/Canonical/Content delta=NONE**；Quick Gate 复跑 PASS（246/246+11/11+audit fresh）。待规划 AI Gate Review → S5-WO-02（Multiple-Link Domain Core，首个 runtime 工作令）。（本轮提交：A=主体本提交 / B=本行所在回填提交——hash 由回填写入）
...
| S5 Build Identity & Itemization Depth（DIR-1） | **AUTHORIZED FOR PHASE 0（2026-09-09 导演原子批准；WO-01 合同锁定已执行）** | 授权恰={BL-002.A1 新词缀（N=4，17→21）+ BL-021.A2 多连接组（max 2）}；显式拒绝=孔颜色/宝石等级品质（负向条件）；计划=`docs/reviews/S5/S5_PLAN.md`；合同=`S5_LINK_CONTRACT.md`；清单=`S5_AFFIX_ADMISSION.md`；台账=`S5_SCOPE_LEDGER.md`；**WO-01（Phase 0 纯文档）已执行待规划 AI Gate Review → S5-WO-02 Multiple-Link Domain Core（首个 runtime 工作令）**；Capability delta=NONE（未实现）；硬停止对其它能力全部延续 |
