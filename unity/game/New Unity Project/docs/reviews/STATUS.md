...
本轮 commit（S4-P3-AFFIX-APPLICABILITY-BREADTH）：**Affix applicability 单一 truth 收口**（Branch B：AffixDef.AllowedSlots bit mask，0=不限槽=旧 13 词缀行为零变化；唯一 canonical predicate=`AffixDef.IsApplicable`，Drop/RollItem+TryRandomCraft+TryDirectedCraft+Audit+Report 全复用零平行判定）+ **词缀 +4**（IDs 13-16 追加：迅握/锋锐=Gloves 专属，壁垒/韧脉=Belt 专属；全部复用已有 StatId/ModOp，运行期消费实证；StatId/ModOp delta=0）+ RollItem=slot-eligible 池（canonical 顺序无重复选取，禁 HashSet 序）+ Directed Craft 非法组合 deterministic reject（不消耗不半写入）+ Audit 扩展（mask 合法性/stable ID 漂移/六槽空池）+ S3 词缀护栏被 S4-P3 护栏取代（13 基线+追加 ≤4=上限 17）+ Production Report 新增 affixApplicability 段（17/13/4，bySlot 13/13/13/13/15/15 全派生 predicate）+ EditMode +10（242/242）+ PlayMode +1（11/11）+ 生成 smoke（~960 物品零非法对+同 seed 可复现）。Gate：SelfTest+EditMode+PlayMode+Audit fresh+Build+PlayerRun exit=0 全 PASS；Performance=NO（低频 loot/craft 路径）、Art Gate=N/A（美术 0 diff）。S4 Phase 3=COMPLETE；Phase 4 未自行启动；S3=COMPLETE；Voice=DEFERRED BY DIRECTOR
...
| S4 Production Scale & Itemization Breadth | **IN PROGRESS** | 规划=`docs/reviews/s4/S4_PLAN.md`；Phase 0/1/2=COMPLETE；**Phase 3=COMPLETE（Itemization Breadth v1：Affix applicability 单一 predicate + 4 条 Gloves/Belt 专属词缀；评审=`docs/reviews/s4/S4_P3_AFFIX_BREADTH_REVIEW.md`）**；Phase 4（Production Simulation）=NOT STARTED 待协调席工作令 |
...
- **S4 已开工（导演 2026-09-09 指令「开启 S4 工作 + 读取仓库 https://github.com/zqs1223041447/GAME-ZZZ」→协调席 S4 Master Plan；无人值守自动执行）**：S4=Production Scale & Itemization Breadth；Phase 0/1/2/3=COMPLETE（P3=Itemization Breadth v1：applicability 单一 predicate+4 条 Gloves/Belt 专属词缀）；**Phase 4（Production Simulation 10k）不得自行启动，待协调席下一工作令**；Stage0 锁全部延续；Voice 继续 DEFERRED；规划=`docs/reviews/s4/S4_PLAN.md`。
