...
本轮 commit（Phase 3 UI 第 3 轮）：A=490ff69（feat(ui)：统一 Tooltip SliceTooltipModel 纯表现+SliceTooltipLayout 纯几何+单一渲染器+装备同槽 union 对比+Support canonical 兼容提示）+ 0226c38（test(ui)：SliceTooltipTests +11=138/138）+ ab1db82（docs(ui)：R3 复核 PASS+方案页/RUNTIME 同步+DECISIONS 规则⑪）；上轮第 2 轮 A=b165e3b+5848857+14b39ca 已回填于下方绿灯表 / B=回填本行
