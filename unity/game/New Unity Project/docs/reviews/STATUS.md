...
本轮 commit（Phase 5 Art Trial R4）：A=d32955e（docs(art)：初筛报告代表怪状态行更新，Wolf=真实 mismatch 留档）+ eecdb97（art(s3)：Lion Head Monster 素材集导入——FBX+D/N/S 贴图+URP 材质+五态控制器）+ 7da8426（feat(art)：第二敌人视觉经通用 EnemyVisualCatalog 正式接入默认 Runtime（Stinger→FireLionVisual），去 Brute 硬编码；并修复 R3 遗留 Mount scale 覆写缺陷——挂载保留预制体根 scale，实测狮 1.400m 贴地/巨魔 2.499m）+ c2b2d21（test(art)：+7 EditMode 契约+1 PlayMode 真实运行；资源契约 REQUIRED 7/7→8/8；Mount scale 断言随根因修复同步）+ c63552b（docs(art)：R4 复核 PASS—SECOND ENEMY VISUAL INTEGRATED）；上轮 Phase 5 Art Trial R3 A=ec48d67（feat）+ 938099a（test）+ b921706（docs）已回填于下方绿灯表 / B=回填本行
