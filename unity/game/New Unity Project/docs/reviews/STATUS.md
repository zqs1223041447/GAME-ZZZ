...
本轮 commit：A=b3999cd（tooling：Performance Gate `-IncludePerformance` 第四层 + 硬件锁定契约 `docs/qa/PERFORMANCE_GATE.json` + Harness `-arenaPerfGate` 钉帧率与硬件证据行；健壮性修复 stale-lock 清理/playInfra 绑定/Depth8）+ 0eeeea8（docs：`PERFORMANCE_BUDGET.md` 预算语义 + QA 四层化 + DECISIONS M7 长期规则⑤-⑧）+ 422d356（docs：S2P 1440p/120 CLOSED + 冻结证据包 `1440p-120-m7/` + M7 复核 PASS）/ B=回填本行
