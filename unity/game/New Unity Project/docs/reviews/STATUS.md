...
本轮 commit（M8）：A=ead815c（tooling：显式性能证据快照 Operator `tools/snapshot_performance_evidence.ps1`，SelfTest 9/9）+ B=3a577d3（docs：M7 后 1440p 真相修正 + QA 四层命令块/Evidence Capture 节 + DECISIONS 规则⑨ Verifier≠Historian）+ a11aa87（chore：ProjectSettings define 振荡矩阵实测收敛至 Performance 门终态=含 define；92a71a9 为其前置试探已被推翻，历史保留）+ 9889998（docs(s2p)：M8 证据冻结 gate-a/gate-b 各 14 文件 + SUMMARY）/ D=回填本行
