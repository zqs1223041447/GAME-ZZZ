...
本轮 commit（Phase 5 Art Trial R2）：A=d488650（art(s3)：Dexsoft Troll Warrior 单模型最小导入 GUID 保持+Humanoid rig vendor 预切 clips 五类零 MISSING+URP/Lit PBR 迁移（BaseMap+Normal+Spec）+TrollWarriorTrial wrapper（零 gameplay 组件）+贴地/bind 2.15m 实测 2.35m≈1.8-2.0x DarkKnight）+ d9c02bc（test(art)：TrollWarriorArtTrialTests +10=158/158）+ e3c250f（docs(art)：R2 复核 ACCEPT WITH FOLLOW-UP+初筛报告代表怪行 Troll_2 判定更新）；上轮 Phase 5 Art Trial R1 A=e5f646e+ed5c4a8+415ae41 已回填于下方绿灯表 / B=回填本行
