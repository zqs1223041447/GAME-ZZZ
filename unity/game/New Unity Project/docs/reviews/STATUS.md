...
本轮 commit（Phase 5 Art Trial R5）：A=1dd6405（feat(art)：正式敌人视觉状态反馈——canonical Hit/Ignite truth 消费+MaterialPropertyBlock tint，Hit>Ignite>Normal、原色缓存精确恢复、仅状态变化写 renderer、零 gameplay 写/零材质实例，IGNITE_TINT_NOT_PRESENTED 缺口收口）+ 9b88403（test(art)：+8 EditMode 反馈契约+2 PlayMode 真实运行（Troll Ignite DoT 不变性/FireLion Hit 优先恢复），时间窗全按模拟值判定）+ f71ee14（docs(art)：R5 复核 PASS—FORMAL ENEMY VISUAL FEEDBACK PARITY ESTABLISHED+DECISIONS 规则⑬）；上轮 Phase 5 Art Trial R4 五笔（d32955e+eecdb97+7da8426+c2b2d21+c63552b）已回填于下方绿灯表 / B=回填本行
