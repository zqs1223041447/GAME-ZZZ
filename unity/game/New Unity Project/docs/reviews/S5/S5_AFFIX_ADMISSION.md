# S5_AFFIX_ADMISSION — 有界词缀广度准入清单（S5-WO-01 锁定；AC-10/11/12 交付物）
...
| 2 | `AffixId.Ironhide`(18) | 铁骨 | `Armour` / `Increased` | 0.10–0.22 | Weapon/Body/Helmet/Boots/Gloves（**排除 Belt**——保留「壁垒」Belt 专属生态位） | PoE "% increased Armour" 本地护甲词缀族 | 与「壁垒」同 stat/op 轴（壁垒=Belt 专属）；Armour Increased 已消费（壁垒在用） | 防御构筑轴（非 Belt 槽获得 %甲） |
...
| 4 | `AffixId.Expanse`(20) | 开阔 | `AreaRadiusMore` / `More` | 0.10–0.20 | 不限槽（mask=0） | PoE "increased Area of Effect" 词缀族（内部对应既有 `AreaRadiusMore` More 轴——Area 技能与「集中」Support 同轴消费） | ModOp.More ≤ Override 且为既有语义（战斗数学 moreProduct；词缀行校验允许 ≤Override）；AreaRadiusMore 为已消费 stat | RC-A（范围构筑）核心追求词缀 |
