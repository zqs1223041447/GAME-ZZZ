...
| Socket / Link | Partial | RUNTIME.md Support/Socket 节（Weapon 3S→Q、Body 3S→W、Helmet 2S→E、Boots 0S；一条 Link；无孔色；Gloves/Belt 各 1 孔仅展示计数、不映射技能孔位——SupportCapacity 只走 Weapon/Body/Helmet） | **S5 跟踪（2026-09-09，WO-06 全收口门 PASS 后最终事实）**：Multiple Link Groups=**SUPPORTED**（域核 S5-WO-02+集成 S5-WO-03（ACCEPT）+验证 S5-WO-05（ACCEPT）+Production Closure S5-WO-06（ProdSim×3 exact hash=FNV1A64:9a4c9524d0b3e214；canonical+Art 性能门锁定硬件双 PASS；EditMode 314/314）——子项独立记录）；Socket Color=**UNSUPPORTED/NOT AUTHORIZED（导演显式拒绝，S5）**；Gem Level/Quality=**UNSUPPORTED/NOT AUTHORIZED（导演显式拒绝，S5）**；**Aggregate Socket/Link=PARTIAL（不因 Multi-Link 完成整体晋升）** |
...
| Mastery | **Partial** | S6P-WO-03：显式选择 + 官方簇 Notable 前置 + 原子提交；22/1863 choice 当前可兑现（其余 fail-closed）；静态 Traversal 仍 SPECIAL_BLOCKED，专精不是 transit | Jewel/Timeless/多可兑现 choice/重选经济=Unsupported；Ascendancy 仍 Director-gated |
| Ascendancy | **Unsupported** | 无数据无 runtime | BDA Phase 3 范围，Director-gated |
| Equipment / Affix | Supported | 6 槽全闭环（drop/equip/replace/聚合/快照/tooltip/抽屉 2×3）+ 单一 applicability truth + 双行词缀；`SixSlotEquipmentTests`、`AffixApplicabilityTests`、`SliceDrawerTests` | 新槽/新词缀族=Forbidden（未授权）；Ring/Offhand/Amulet=NOT AUTHORIZED。**S5 跟踪（2026-09-09）**：Affix breadth 17→**21**（S5-WO-04，BL-002.A1 锁定清单恰 4 条=迅疾/铁骨/睿智/坚韧，全部复用既有 stat/modop/applicability/生成路径，无新 capability family；铁骨 Belt 排除=GAME-ZZZ 有界适用性决策）；Socket/Link 行见上（Multi-Link 集成已实现，Socket Color 未授权）。**S5-WO-05 验证联动（2026-09-09，仅 evidence linkage 不晋升）**：RC-M/RC-P/RC-A validation scenarios 全过（候选保持 NOT LOCKED）；EditMode 314/314。**S5-WO-06 收口（2026-09-09）**：全门 PASS 候选——ProdSim×3 hash exact（FNV1A64:9a4c9524d0b3e214）+canonical/Art 双性能门锁定硬件 PASS+视觉解析 4/4；Affix breadth=21 entries（既有 family 语义）；**Director Final Gate=APPROVED（2026-09-12）**；正式 **S5=COMPLETE**（本批准不新增 capability family，不授权 Flask/Jewel/Ascendancy/Timeless/新 Stat 轴） |
