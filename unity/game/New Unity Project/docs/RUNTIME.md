...
- 玩家视图：`Resources/Player/DarkKnight`（Dark Knight 换模，视图层动画 Idle/Run/Attack/Cast），无预制体时**回退 S1 胶囊**（旧 Eve 回退分支已删除；自制 Eve FBX 已剔除）。模型按**身体包围盒**每帧贴地（`EveView.UpdateGround`，武器长刀不参与身高/贴地计算），身体净高 ≈1.19。根上留 CapsuleCollider（贴地/墙用，高 1.16），不挡怪。
